...
void init_udp_socket() {
    ...
    int b = bind(udp_socket, (const struct sockaddr*) &addr, sizeof(addr));
    if (b == -1) {
        printf(YEL"bind error\n"RST);
        exit(1);
    }

    printf("[%s]: init_udp_socket success\n", client_name);
}
...
int main(int argc, char** argv) {
    if (argc != 6) {
        ...
        printf("./client_udp  client_name  port_to_listen  ");
...
void handle_token() {
    ...
    ssize_t bytes_read = recvfrom(udp_socket, &token, sizeof(token),
        0, (struct sockaddr*) &addr, (socklen_t*) &addrlen);
    if (bytes_read != sizeof(token)) {
        printf(YEL"recvfrom error\n"RST);
        return;
    }
    //log
...
void forward_token() {
    ...
    ssize_t bytes_send = sendto(udp_socket, &token, sizeof(token),
...
void send_log(char* message, int size) {
    ...
    int st = sendto(log_socket, message, size,
